...
## 2026-02-26 (Part 3)
- **任务**: 修复 NVIDIA 加速卡计数不一致的问题
- **问题**: 在某些情况下，`nvidia-smi` 输出的空行或无效行（字段不足）会被计入总行数 `len(gpus)`，但不会被解析并加入 `details` 列表，导致返回的 `accelerator_count` 大于实际的卡详情数量（例如总数显示 9，但列表只有 8 张卡）。
- **变更**:
    - **后端 (`backend/services/monitor_service.py`)**: 修改 `parse_nvidia_output` 函数，将 `accelerator_count` 的计算源从原始行数 `len(gpus)` 改为解析后的列表长度 `len(details)`，确保计数绝对准确。
- **验证**:
    - 检查代码逻辑，确认计数直接依赖于最终生成的详情列表。
    - 预期结果：前端“加速卡详情”中的【总数】应始终与【卡状态】列表的条目数保持一致。

## 2026-03-11
- **任务**: 新增 IBMC 账号列
- **变更**:
    - **后端**:
        - 修改 `Machine` 模型 (`backend/models.py`)，新增 `ibmc_username` 字段。
        - 编写并执行迁移脚本 `migrate_add_ibmc_username.py`，更新数据库表结构。
    - **前端**:
        - 在机器列表表格中新增 "IBMC 账号" 列，位于 "IBMC IP" 和 "IBMC 密码" 之间。
        - 更新 "新增机器" 和 "编辑机器" 对话框，加入 `ibmc_username` 输入框。
        - 实现 `handleIbmcUsernameBlur` 逻辑，支持即时保存。
        - 更新导入/导出功能，支持 `ibmc_username` 字段。
    - 重新编译前端。
- **验证**:
    - 数据库迁移成功，无报错。
    - 前端页面正确显示 "IBMC 账号" 列。
    - 新增/编辑机器时可输入并保存 IBMC 账号。
    - 列表页直接编辑 IBMC 账号可自动保存。

## 2026-03-11 (Part 2)
- **任务**: 优化 IBMC 相关操作体验
- **变更**:
    - **前端 (`frontend/src/App.vue`)**:
        - **IBMC 账号复制**: 为 "IBMC 账号" 列添加了复制按钮，复用 `copyToClipboard` 方法，并添加了 `@mousedown.prevent` 以防止触发 `blur` 保存逻辑。
        - **IBMC IP 跳转**: 为 "IBMC IP" 列添加了跳转按钮（`Link` 图标）。
        - **IP 校验**: 新增 `isValidIp` 函数，使用正则校验 IP 格式，仅在 IP 有效时显示跳转按钮。
        - **快捷访问**: 新增 `openIbmcPage` 函数，点击跳转按钮将在新标签页打开 `https://<IP>/`。
        - **图标引入**: 引入了 `Link` 图标。
        - **修复 Bug**: 修复了之前提交中 `handleIbmcUsernameBlur` 的拼写错误（多余的空格）。
    - 重新编译前端。
- **验证**:
    - "IBMC 账号" 列现在显示复制图标，且点击后能正确复制内容且不触发保存。
