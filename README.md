...
4.  **可视化拓扑**：
    ...
    *   **交互式过滤**：支持通过点击图例开关 CPU、GPU、网络、存储等不同类型节点的显示。
    *   **实时同步**：预览拓扑时，后台数据变更将自动触发前端视图的无感刷新。
5.  **系统特性**：
    *   **版本管理**：实时展示前端版本及 Topo 解析子模块的 Git Commit ID，确保版本一致性。
    *   **定时检测**：可自定义检测频率。
    *   **数据持久化**：使用 SQLite 数据库。
...
*   默认端口为 9000，如果被占用请修改 `backend/main.py` 中的端口或设置环境变量 `PORT`。
*   如需重新编译前端代码：
    ```bash
    cd frontend
    npm run build
